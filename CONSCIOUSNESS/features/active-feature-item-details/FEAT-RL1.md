...
## Acceptance Criteria
- [ ] **AC-1** — Each notebook runs top-to-bottom on Colab free tier with no local-only dependencies
- [ ] **AC-2** — A pip-install cell provisions Gymnasium, PyTorch, and Stable-Baselines3 as needed
- [ ] **AC-3** — Plots and environment renders display inline in the notebook
- [ ] **AC-4** — Long-running training cells expose a reduced-timestep quick-run path
