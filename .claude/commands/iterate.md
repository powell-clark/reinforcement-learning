...
- **APPROVED** — conclusive: correct, tested red→green, CI green, ready to merge.
- **CONTINUE DEVELOPMENT** — on the right track but not finished. State the next concrete step.
- **DISAPPROVED** — no viable path: the approach is a dead end. Say why.
...
- Do not use destructive git operations. Never push to a protected branch.
- Do not touch protected paths: `CHANGELOG.md`, active ADRs, `.claude/settings.json`,
  anything under `~/.claude/plugins/`.
