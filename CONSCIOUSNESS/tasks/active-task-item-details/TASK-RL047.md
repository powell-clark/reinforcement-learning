...
- [x] **AC1** — Insert a step-by-step derivation of the Bellman expectation equation for `V^π(s)` in `1a_mdp_theory.ipynb`'s Bellman-equations section (cell 12 or new cells at that point): define `G_t = R_{t+1} + γR_{t+2} + ...`, show the recursive form `G_t = R_{t+1} + γG_{t+1}`, define `V^π(s) = E_π[G_t | S_t=s]`, and apply linearity of expectation plus the law of total expectation (condition on action via `π(a|s)`, then on next state/reward via `P(s',r|s,a)`) to arrive at the given closed-form `V^π(s)` equation. Evidence: commit ebe2a15, notebook cell 12.
- [x] **AC2** — Parallel derivation for `Q^π(s,a) = E_π[G_t | S_t=s, A_t=a]`, arriving at the given closed-form Bellman expectation equation for `Q^π`, and showing how `V^π(s)` and `Q^π(s,a)` relate (`V^π(s) = Σ_a π(a|s) Q^π(s,a)`). Evidence: commit ebe2a15, notebook cell 12.
- [x] **AC3** — Derive the Bellman optimality equations from `V*(s) = max_π V^π(s)` and the identity `V*(s) = max_a Q*(s,a)`, showing how substituting into the Bellman expectation equation for `Q` yields the given `V*`/`Q*` optimality formulas. Evidence: commit ebe2a15, notebook cell 12.
- [x] **AC4** — Execute `1a_mdp_theory.ipynb` top-to-bottom so cell 15 (`MDPSolver` + `student_mdp` value-iteration driver) has real stored output (non-empty `outputs`, non-`None` `execution_count`); the printed optimal value function and optimal policy must be sanity-checked against the Student MDP's reward structure (evidence in commit message / task card, not just an execution flag). Evidence: commit ebe2a15 — cell 15 execution_count=7, converged in 154 iterations, V(S)=42.1053, V(L)=100.0000, π(S)=π(L)=Study. Hand-verified exactly: under "always Study" state L self-loops paying reward 10/step so V(L)=10/(1-0.9)=100; state S solves V(S) = -1 + 0.9*(0.9*V(S)+0.1*100) = 8/0.19 = 42.1053 — both match to 4 decimal places.
- [x] **AC5** — Notebook executes clean top-to-bottom with zero error cells after both changes, verified by an independent nbformat error-cell scan (not just the executor's own exit message), matching the `total error cells: N / total cells: M` pattern used for TASK-RL046. Evidence: independent nbformat scan, 1a_mdp_theory.ipynb 0/7 error cells.
